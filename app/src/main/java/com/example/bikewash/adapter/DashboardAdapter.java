...
import static com.example.bikewash.utility.Constants.START;
...
public class DashboardAdapter extends RecyclerView.Adapter<DashboardAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (washing_status != null && !washing_status.equalsIgnoreCase( "" )) {
            ...
            } else if (washing_status.equalsIgnoreCase( START )) {
                holder.doneButton.setText( washing_status );
        ...
        holder.doneButton.setOnClickListener( v -> {
            if (washing_status.equalsIgnoreCase( PENDING )) {
                ...
                mDatabase.child( ALL ).child( SharePreference.getKey( context ) ).child( WASHING_STATUS ).setValue( START );
            } else if (washing_status.equalsIgnoreCase( START )) {
                FirebaseDatabase database = FirebaseDatabase.getInstance();
                DatabaseReference mDatabase = database.getReference();
                mDatabase.child( ALL ).child( SharePreference.getKey( context ) ).child( WASHING_STATUS ).setValue( COMPLETED );
            } else if (washing_status.equalsIgnoreCase( COMPLETED )) {
